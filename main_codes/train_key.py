...
training_dataset_loaded = [
    ...
    for row in loaded_data.values if row[0] == application_name
]
...
def project_qvalues_cql_sampled(env, s, a, target_values, network, target_network,
    ...
    preference = torch.as_tensor(preference, dtype=torch.float32, device=pred_sa.device)  # [B,2]

    # Element-wise multiply and sum to scalarize
    pred_scalar = (pred_sa * preference).sum(dim=1, keepdim=True)  # [B,1]
    loss = F.smooth_l1_loss(pred_scalar, target_values)    # target_values [B,2]
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
...
def q_backup_sparse_sampled(env, network, s, a, ns, r, discount=0.99, preference=None, **kwargs):
    ...
    dot = (prefs_flat * q_flat).sum(dim=1)                                          # [B*A]
    score = (dot).view(B, A)                                                  # [B,A]

    act = score.argmax(dim=1)                                    # [B]
    # best_q_vec = target_q.view(B, A, 2)[torch.arange(B, device=device), act]   # [B,2]
    best_q_vec = score[torch.arange(B, device=device), act].unsqueeze(1)  # [B,2]


    # r_t = torch.as_tensor(r, dtype=torch.float32, device=device) # [B,2]
    preference = torch.as_tensor(preference, dtype=torch.float32)
    r = torch.as_tensor(r, dtype=torch.float32)

    # Element-wise multiply and sum along dim=1
    r_t = (preference * r).sum(dim=1, keepdim=True)
    # print(r_t.shape)  # torch.Size([128, 1])
  ...
  for i in range(num_itrs):
    for j in range(project_steps):
      ...
      loss = project_qvalues_cql_sampled(
          ...
          cql_alpha=cql_alpha, weights=None, preference=preference
      )
      # scheduler.step(loss)
      scheduler.step()
...
with open(output_file_path, 'w', newline='') as csvfile:
    ...
    trained_net = conservative_q_iteration(env, network, fig, axs,
                                        ...
                                        preference = preferences, interpolator = None, w_batch_interp=None, args=args)
...
def compress_results_folder(results_dir,current_time,cql_alpha,LR):
    tar_file_name = f'{results_dir}/training_results_{current_time}_all_preference_{preference}_{application_name}.tar'
