...
def denormalize(data,MIN,MAX):
    return np.round((np.float64(data)*(MAX-MIN) + MIN), decimals=1)
...
ACTION_MAX = max(ACTIONS)                                                                                                    # Minima of control space
ACTIONS = normalize(ACTIONS, ACTION_MIN, ACTION_MAX)                                                                                                  # Maxima of control space
PROG_MAX = 300.0                                                                                                           # Maxima of observation space (performance)
PROG_MIN = 0.0    
...
def process_callback(states):
    progress = normalize(measure_progress(states['progress']),PROG_MIN,PROG_MAX)
    measured_power = normalize(compute_power(states['energy_0'],states['energy_1']),ACTION_MIN,ACTION_MAX)
    PAPI = collect_papi(states)
    # Concatenate the progress, measured_power, and PAPI lists
    combined_data = [progress, measured_power] + PAPI + [np.float64(decay_factor)]
        ...
        while True:
            ...
            if current_time - last_pcap_change >= 3:
                ...
                if 'state_dict' in globals() and state_dict and state_dict != reference_lib:                    
                    ...
                    PCAP = ACTIONS[argmax]
                    print(f"Out before normalization is {PCAP}")
                    # PCAP = min(ACTIONS, key=lambda x: abs(x-PCAP))
                else: 
                    print("choosing sane value")
                    PCAP = normalize(78.0,ACTION_MIN,ACTION_MAX)
                PCAP = denormalize(PCAP,ACTION_MIN,ACTION_MAX)
                print(PCAP)
