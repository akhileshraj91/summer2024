...
parser.add_argument('--actuate', action='store_true', help='Enable actuation (disabled by default)')

args = parser.parse_args()

# Actuation disabled by default. Pass --actuate to enable physical actuator calls.
ACTUATE = bool(args.actuate)
    ...
    with open(f'{EXP_DIR}/{APPLICATION}_output.log','w') as log_file, open(f'{EXP_DIR}/frequency.csv', mode='w', newline='') as frequency_file, open(f'{EXP_DIR}/measured_power.csv', mode='w', newline='') as power_file, open(f'{EXP_DIR}/progress.csv', mode='w', newline='') as progress_file, open(f'{EXP_DIR}/energy.csv', mode='w', newline='') as energy_file, open(f'{EXP_DIR}/FREQ_file.csv', mode='w', newline='') as FREQ_file, open(f'{EXP_DIR}/papi.csv', mode='w', newline='') as papi_file:
        ...
        while True:
            ...
            if current_time - last_pcap_change >= 2:
                ...
                actuator_used = None
                if ACTUATE and actuators:
                    # Only attempt actual actuation if explicitly enabled
                    try:
                        actuator_used = actuators[1] if len(actuators) > 1 else actuators[0]
                        client.actuate(actuator_used, FREQ)
                    except Exception as e:
                        print(f"[WARN] failed to actuate frequency: {e}")
                        actuator_used = None
                else:
                    # No actuation by default
                    print("[INFO] Actuation disabled; not changing CPU frequency.")
                FREQ_time = time.time()
                # write actuator UUID if used, otherwise 'none'
                FREQ_writer.writerow([FREQ_time, actuator_used if actuator_used is not None else 'none', FREQ])
    ...
    if args.experiment == 'random':
        for REPEAT in range(repeat):
            ...
            for APPLICATION in APPLICATIONS:
                experiment = 'ondemand_governor_data'
