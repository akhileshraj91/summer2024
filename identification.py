...
def logs_conf_func():
    ...
    if os.path.exists(f'./experiment_data/{WORKLOAD}_identification'):
        pass
    else:
        os.makedirs(f'./experiment_data/{WORKLOAD}_identification')
        with open(f'./experiment_data/{WORKLOAD}_identification/{LOGGER_NAME}.log', 'w') as file:
            file.write('')
