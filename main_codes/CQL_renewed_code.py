...
# %%
def project_qvalues_cql_sampled(env, s, a, target_values, network, optimizer, cql_alpha=0.1, num_steps=50, weights=None, preference=None):
...
def conservative_q_iteration(env,
  ...
  scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=1000, min_lr=1e-8, threshold=0.01)
  ...
  for i in range(num_itrs):
    for j in range(project_steps):
      ...
      target_values = q_backup_sparse_sampled(env, network, s, a, ns, r, preference=prefs, **kwargs)
